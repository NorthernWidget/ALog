...
{\bfseries interrupt\+\_\+pin\+\_\+number} is the digital pin number corresponding to the appropriate interrupt; it uses the Arduino digital\+Pin\+To\+Interrupt(n\+\_\+pin) function to properly attach the interrupt

{\bfseries reading\+\_\+duration\+\_\+milliseconds} How long will you count revolutions? Shorter durations save power, longer durations increase accuracy; very long durations will produce long-\/term averages. Typical values are a few seconds.

{\bfseries meters\+\_\+per\+\_\+second\+\_\+per\+\_\+rotation\+:} Conversion factor between revolutions and wind speed. For the Inspeed Vortex wind sensor that we have used (\href{http://www.inspeed.com/anemometers/Vortex_Wind_Sensor.asp}{\tt http\+://www.\+inspeed.\+com/anemometers/\+Vortex\+\_\+\+Wind\+\_\+\+Sensor.\+asp}), this is\+: {\bfseries 2.\+5 mph/\+Hz = 1.\+1176 (m/s)/\+Hz}
...
{\bfseries command} is the instruction code sent to the Atlas Scientific product. See the data sheet for your specific sensor.

{\bfseries soft\+Ser\+Rx} is the software serial receive port

{\bfseries soft\+Ser\+Tx} is the software serial transmit port

{\bfseries baud\+Rate} is set by the Atlas Scientific sensor\textquotesingle{}s baud rate.

{\bfseries print\+Return} is true if you determines whether you care about (i.\+e. want to print) the Serial response, and false if you would just like to clear the buffer.

{\bfseries save\+Return} is true if you want to save the response to the SD card.
...
Reads a Decagon Devices 5\+TE soil moisture probe. N\+E\+E\+DS T\+E\+S\+T\+I\+NG with current A\+Log version.
...
{\bfseries excit\+Pin} activates the probe and powers it

{\bfseries data\+Pin} receives incoming serial data at 1200 bps
...
{\bfseries A\+D\+C\+\_\+resolution\+\_\+nbits} (10-\/16 for the A\+Log Bottle\+Logger) is the number of bits of A\+DC resolution used (oversampling for $>$10)
...
{\bfseries Rref\+\_\+therm} is the value of the reference resistor that you use with the built-\/in thermistor (reference resistor supplied separately, placed in appropriate slot in header)
...
{\bfseries x\+Pin} Analog pin number corresponding to x-\/oriented tilts
...
{\bfseries \+\_\+logger\+\_\+name\+:} Name associated with this data logger; often helps to relate it to the project or site

{\bfseries \+\_\+filename\+:} Name of main data file saved to SD card; often helps to relate it to the project or site; used to be limited to 8.\+3, but now is not.

{\bfseries \+\_\+day\+Interval\+:} How many days to wait before logging again; can range from 0-\/6.

{\bfseries \+\_\+hour\+Interval\+:} How many hours to wait before logging again; can range from 0-\/24.

{\bfseries \+\_\+min\+Interval\+:} How many minutes to wait before logging again; can range from 0-\/59.

{\bfseries \+\_\+sec\+Interval\+:} How many seconds to wait before logging again; can range from 0-\/59.
...
{\bfseries linpot\+Pin} Analog pin number

{\bfseries Rref} Resistance value of reference resistor \mbox{[}ohms\mbox{]}

{\bfseries slope} Slope of the line (distance = (slope)R + R0)

{\bfseries intercept} (R0) of the line (distance = (slope)R + R0)
...
{\bfseries Ex} Excitation pin that turns the sensor on; if this is not needed (i.\+e. you are turning main power off and on instead), then just set this to a value that is not a pin, and ensure that you turn the power to the sensor off and on outside of this function

{\bfseries npings} Number of pings over which you average; each ping itself includes ten short readings that the sensor internally processes

{\bfseries write\+All} will write each reading of the sensor (each ping) to the serial monitor and SD card.

{\bfseries max\+Range} The range (in mm) at which the logger maxes out; this will be remembered to check for errors and to become a nodata values

{\bfseries R\+S232} this is set true if you use inverse (i.\+e. R\+S232-\/style) logic; it works at standard logger voltages (i.\+e. it is not true R\+S232). If false, T\+TL logic will be used.
...
{\bfseries analog\+Pin} is the pin that receives the amplified voltage input

{\bfseries raw\+\_\+m\+V\+\_\+per\+\_\+\+W\+\_\+per\+\_\+m2} is the conversion factor of the pyranometer\+: number of millivolts per (watt/meter$^\wedge$2). This does not include amplification!

{\bfseries gain} is the amplification factor

{\bfseries V\+\_\+ref} is the reference voltage of the A\+DC; on the A\+Log, this is a precision 3.\+3V regulator (unless a special unit without this regulator is ordered; the regulator uses significant power)
...
{\bfseries vane\+Pin} is the analog pin that reads the wind vane resistance
